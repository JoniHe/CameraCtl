...

{
    ...
    private static string _logDirectory;
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(info).ToUpper()}  {info}");
            var log = new LogInfo()
            {
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(info).ToUpper()}   {source}  {info}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Info,
                Message = info,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(info).ToUpper()}   {source.FullName}  {info}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Info,
                Message = info,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(debug).ToUpper()}   {debug}");
            var log = new LogInfo()
            {
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(debug).ToUpper()}   {source}  {debug}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Debug,
                Message = debug,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(debug).ToUpper()}   {source.FullName}  {debug}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Debug,
                Message = debug,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(error).ToUpper()}   {error.Source}  {error.Message}{Environment.NewLine}{error.StackTrace}");
            var log = new LogInfo()
            {
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(error).ToUpper()}   {source.FullName}  {error.Message}{Environment.NewLine}{error.StackTrace}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Error,
                Message = error.Message,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(error).ToUpper()}   {source.FullName}  {error}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Error,
                Message = error,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(error).ToUpper()}   {source}  {error.Message}{Environment.NewLine}{error.StackTrace}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Error,
                Message = error.Message,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(error).ToUpper()}   {source}  {error}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Error,
                Message = error,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(fatal).ToUpper()}   {fatal.Source}  {fatal.Message}{Environment.NewLine}{fatal.StackTrace}");
            var log = new LogInfo()
            {
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(fatal).ToUpper()}   {source.FullName}  {fatal.Message}{Environment.NewLine}{fatal.StackTrace}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Fatal,
                Message = fatal.Message,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(fatal).ToUpper()}   {source.FullName}  {fatal}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Fatal,
                Message = fatal,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(fatal).ToUpper()}   {source}  {fatal.Message}{Environment.NewLine}{fatal.StackTrace}");
            var log = new LogInfo()
            {
                LogLevel = LogLevel.Fatal,
                Message = fatal.Message,
        ...
        {
            LogQueue.Enqueue($"{Now}   [{Thread.CurrentThread.ManagedThreadId}]   {nameof(fatal).ToUpper()}   {source}  {fatal}");
